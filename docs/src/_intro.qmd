...
loss(yhat, y) = Flux.Losses.mse(yhat, y)

# Training:
opt = Flux.Adam(1e-3)
t = Optimisers.setup(opt, nn)
epochs = 1000
avg_loss(data) = mean(map(d -> loss(nn(d[1]), d[2]), data))  # Fixed this line!
...
loss(yhat, y) = Flux.Losses.logitbinarycrossentropy(yhat, y) 
